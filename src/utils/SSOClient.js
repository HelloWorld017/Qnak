...
class Client {
	...
	static _validateSign(payload, timestamp, sign) {
		const {
			signClient,
			timeClient
		} = this._signPayload(payload, false);
	...
	getLogoutUrl(sid, redirectUri) {
		...
		*/
		const {
			sign,
			timestamp
		} = this._signPayload([sid, redirectUri]);
		const params = {
			client_id: this.clientId,
			sid,
			timestamp,
	...
	async modifyPoint(sid, delta, message, lowerBound = 0) {
		...
		*/
		const {
			sign,
			timestamp
		} = this._signPayload([sid, delta, message, lowerBound]);
		const params = {
			client_id: this.clientId,
			sid,
			delta,
	...
	static async getNotice(offset = 0, limit = 3, dateAfter = 0) {
		...
		*/
		...
		try {
			const res = await axios.get(this.URLS.notice, {
				params
			});
			return res.data;
		} catch (err) {
